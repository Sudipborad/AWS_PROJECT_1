...
const OfficerDashboard = () => {
  ...
  const { fetchData, updateData, supabase } = useSupabase();
  ...
  useEffect(() => {
    const fetchOfficerData = async () => {
      if (!userId) return;
        ...
        if (user) {
          setOfficerName(`${user.firstName || ''} ${user.lastName || ''}`.trim() || user.username || 'Officer');
        }
        ...
        let { data: assignedComplaints, error: assignedError } = await supabase
          .from('complaints')
          .select('*')
        ...
        if (assignedError) {
          ...
          console.log("Error fetching by assigned_to, column might not exist:", assignedError);
          assignedComplaints = [];
        }

        console.log("Found explicitly assigned complaints:", assignedComplaints?.length || 0);

        // Also fetch complaints from the officer's area
        const { data: areaComplaints, error: areaError } = await supabase
          .from('complaints')
          .select('*')
          .eq('area', officerArea);

        if (areaError) {
          console.error('Error fetching area complaints:', areaError);
        } else {
        ...
        if ((!assignedComplaints || assignedComplaints.length === 0) && areaComplaints && areaComplaints.length > 0) {
          ...
          assignedComplaints = areaComplaints.filter(c => !c.assigned_to || c.assigned_to === userId);
          
          // Automatically assign the first few complaints to this officer if none are assigned
          if (assignedComplaints.length > 0 && (!assignedComplaints[0].assigned_to)) {
            ...
            for (const complaint of complaintsToAssign) {
              try {
                ...
                if (checkError && checkError.code === '42703') {
                  // Columns don't exist yet, try to create them
                  console.log("Columns don't exist yet. Adding them now...");
                  ...
                  if (alterError) {
                    ...
                    continue; // Skip this complaint and try the next one
                  }
                }
                
                // Now try to update just the assigned_to field first
        ...
        // Calculate stats
        const resolvedCount = assignedComplaints?.filter(c => c.status === 'resolved').length || 0;
        const pendingCount = assignedComplaints?.filter(c => c.status === 'pending').length || 0;
        const inProgressCount = assignedComplaints?.filter(c => c.status === 'in-progress').length || 0;
        const totalCount = assignedComplaints?.length || 0;

        // Calculate progress percentage
        const newProgress = totalCount > 0 ? Math.round((resolvedCount / totalCount) * 100) : 0;
        ...
        if (assignedComplaints && assignedComplaints.length > 0) {
          const formattedComplaints = assignedComplaints
        ...
        const unassignedAreaComplaints = areaComplaints?.filter(c => 
          (!c.assigned_to || c.assigned_to !== userId) && 
          c.status !== 'resolved'
        ) || [];

        if (unassignedAreaComplaints.length > 0) {
          const formattedAreaComplaints = unassignedAreaComplaints
              .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
              .slice(0, 4)
              .map(complaint => ({
                id: complaint.id,
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {stats.resolvedComplaints} of {stats.totalAssignedComplaints} assigned cases resolved this month
              </p>
            </div>
          </motion.div>
        </section>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-2xl font-bold">{stats.totalAssignedComplaints}</h3>
                  </div>
