...
const OfficerProfilePage = () => {
  ...
  useEffect(() => {
    const fetchOfficerData = async () => {
      ...
      try {
        ...
        // Fetch all data
        const [users, allComplaints, allRecyclables] = await Promise.all([
          fetchUsers(),
          fetchComplaints(),
          fetchRecyclables()
        ]);
        ...
        const complaintsDataList = Array.isArray(allComplaints) ? allComplaints : [];
        const recycleDataList = Array.isArray(allRecyclables) ? allRecyclables : [];

        // Fetch officer details
        let officerData = usersData.find((u: any) => u.clerk_id === id);

        if (!officerData) {
          // Try with regular ID if clerk_id doesn't work
          officerData = usersData.find((u: any) => u.id === id);
        ...
        const officerComplaints = complaintsDataList
             .filter((c: any) => c.assigned_to === id || c.assigned_to === officerData.id)
             .sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
             
        setComplaints(officerComplaints);

        // Fetch assigned recycle requests
        const offRecyclables = recycleDataList
            .filter((r: any) => r.area === officerData?.area)
            .sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

        setRecycleRequests(offRecyclables);
      } catch (error) {
  ...
  const name =
    `${officer.firstName || ""} ${officer.lastName || ""}`.trim() || "Unknown";
  const initials = name
    .split(" ")
    .map((n) => n[0])
    .join("")
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Joined{" "}
                      {officer.created_at
                        ? format(new Date(officer.created_at), "MMMM d, yyyy")
                        : "Unknown"}
                    </span>
                  </div>
                </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <Card key={request.id}>
                              <CardContent className="p-4">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    navigate(
                                      `/recyclable-requests/${request.id}`,
                                    )
                                  }
                                >
                                  View Details
                          ...
                          {recycleRequests.length > 5 && (
                            <div className="text-center">
                              ...
                              >
                                View All {recycleRequests.length} Recycle
                                Requests
                              </Button>
                            </div>
                          )}
