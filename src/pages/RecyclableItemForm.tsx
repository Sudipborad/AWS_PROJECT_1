...
const RecyclableItemForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let imageUrl = null;
      if (images.length > 0) {
        console.log("Uploading image to local backend...");
        const file = images[0];
        ...
        if (uploadResponse.data && uploadResponse.data.url) {
          imageUrl = `http://localhost:3001${uploadResponse.data.url}`;
      ...
      console.log("Creating recyclable item record with local API...");
      const insertData = {
        name: itemType,
        description: description || "No description provided",
        quantity: itemWeight ? parseFloat(itemWeight) : 1,
        location: locationString,
        imageUrl: imageUrl, // backend uses imageUrl (camelCase) in destructing but I'll check
        userId: userId || "anonymous",
        status: "pending",
        area: area,
      };
      
      const response = await axios.post("http://localhost:3001/api/recyclables", insertData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
          },
      });
      ...
      if (!response.data) {
        throw new Error("Failed to save item: No response data");
      }
      ...
      toast({
        ...
        description:
          "Your recyclable item collection request has been submitted successfully",
      });
        ...
        >
          <div className="mb-8">
            <h1 className="text-3xl font-bold">
              Request Recyclable Item Collection
            </h1>
            <p className="text-muted-foreground mt-2">
              Provide details about the recyclable items you want to have
              collected
            </p>
          </div>
                        ...
                        {imageUrls.length === 0 ? (
                          <div className="text-center">
                            ...
                            <p className="text-sm text-muted-foreground mb-4">
                              Please add at least one image of the recyclable
                              item
                            </p>
                          ...
                          <div className="w-full">
                            ...
                            <p className="text-sm text-muted-foreground text-center mt-2">
                              {imageUrls.length}/3 images added.{" "}
                              {3 - imageUrls.length} more allowed.
                            </p>
                          </div>
                        )}
                      </div>
