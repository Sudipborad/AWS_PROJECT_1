...
const SchedulePage = () => {
      ...
      try {
        ...
        if (userRole === 'officer' && userId) {
          ...
          if (Array.isArray(complaints)) {
            ...
            const scheduledComplaints = sortedComplaints.slice(0, 2).map((complaint, index) => ({
            ...
            const scheduledRecycling = areaRecyclableItems.slice(0, 1).map(item => ({
  ...
  const handleCompleteTask = async (item) => {
    try {
      ...
      // Update local state
      setScheduleData(prev => prev.map(i => {
        if (i.id === item.id) {
          return {
            ...i,
  ...
  const filteredSchedule = scheduleData.filter(item => 
    ...
    (item.type === 'recycling' ? item.status !== 'collected' : item.status !== 'resolved')
  );
...
const ScheduleItem = ({ item, typeColors, onComplete, userRole }) => {
