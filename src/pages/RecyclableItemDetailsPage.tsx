...
const RecyclableItemDetailsPage = () => {
  ...
  const { fetchRecyclables, fetchUsers, updateRecyclable } = useApi();
  ...
  useEffect(() => {
    const loadRecyclableItem = async () => {
      try {
        ...
        const recyclableData = await fetchRecyclables();
        const foundItem = Array.isArray(recyclableData) ? recyclableData.find((item: any) => item.id === id) : null;

        if (!foundItem) {
          toast({
            title: "Item not found",
            description:
              "The recyclable item you're looking for doesn't exist or has been removed.",
            variant: "destructive",
          });
        ...
        const submitter = Array.isArray(users) ? users.find(
          (u: any) => u.clerk_id === foundItem.user_id,
        ) : null;
        ...
        toast({
          title: "Error",
          description:
            "Failed to load recyclable item details. Please try again.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      console.log(
        `Updating recyclable item ${recyclableItem.id} to status: ${newStatus}`,
      );
      ...
      const result = await updateRecyclable(
        recyclableItem.id,
        updates,
      );
      ...
      toast({
        title: "Item updated",
        description:
          "The recyclable item status has been successfully updated.",
      });
      ...
      toast({
        title: "Error",
        description:
          "Failed to update recyclable item status. Please try again.",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
      ...
      <Layout>
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-muted-foreground">
            The recyclable item you're looking for doesn't exist or has been
            removed.
          </p>
          <Button
            className="mt-4"
      ...
      >
          ...
          <div className="flex items-center gap-2 mt-1">
            <div
              className={cn(
                ...
                statusColors[recyclableItem.status as keyof typeof statusColors]
                  ?.bg,
                statusColors[recyclableItem.status as keyof typeof statusColors]
                  ?.color,
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm">
                          Weight: {recyclableItem.quantity}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <MapPin className="h-4 w-4 text-muted-foreground" />
            ...
            {canEditStatus() &&
              recyclableItem.status !== "collected" &&
              recyclableItem.status !== "cancelled" && (
                <Card>
                  <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm">
                        {formatDate(recyclableItem.date)}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                  ...
                  {recyclableItem.scheduleDate && (
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <Calendar className="h-3.5 w-3.5 text-muted-foreground" />
                        <p className="text-sm">
                          {formatDate(recyclableItem.scheduleDate)}
                        </p>
                      </div>
                    </div>
                  )}
