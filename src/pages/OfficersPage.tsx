...
const OfficersPage = () => {
  ...
  useEffect(() => {
    const fetchOfficers = async () => {
      try {
        ...
        const officerUsersData = await fetchUsersApi();
          ...
          for (const officer of usersData) {
            ...
            const formattedOfficer = {
              ...
              name:
                ...
                "Unknown",
              email:
                officer.email || `${officer.clerk_id || "user"}@example.com`,
              zone: officer.area || "Unassigned",
              status: officer.status || "active",
              joinDate: new Date(
                officer.created_at || Date.now(),
              ).toLocaleDateString(),
              avatar:
  ...
  const filteredOfficers = officers.filter((officer) => {
    ...
    if (activeTab === "active")
      return matchesSearch && officer.status === "active";
    if (activeTab === "inactive")
      return matchesSearch && officer.status === "inactive";
    if (activeTab === "new") {
                      ...
                      <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="flex md:flex-col gap-2 items-center md:items-end justify-end">
                          <Button
                            size="sm"
                            onClick={() =>
                              navigate(`/officer-profile/${officer.id}`)
                            }
                          >
                            View Profile
                          </Button>
