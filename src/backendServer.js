  ...
  try {
    ...
    try {
      await mainPool.query(
        `CREATE DATABASE ${process.env.DB_NAME || "ecoguardian"}`,
      );
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    console.log(
      `📝 Registration attempt: ${email}, name: ${firstName} ${lastName}`,
    );
...
// Recyclables
app.post("/api/recyclables", protectRoute, async (req, res) => {
  try {
    const { name, description, quantity, pickupDate, location, imageUrl } =
      req.body;
    const recyclableId = `recyclable_${Date.now()}`;
    const result = await pool.query(
      `INSERT INTO recyclables (id, name, description, quantity, pickup_date, location, image_url, user_id, status)
...
app.get("/api/recyclables/:id", protectRoute, async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM recyclables WHERE id = $1", [
      req.params.id,
    ]);
    if (result.rows.length === 0)
